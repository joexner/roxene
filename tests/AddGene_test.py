...
class AddGene_test(unittest.TestCase):
    ...
    def test_insertion_at_beginning(self):
        """Test that genes can be inserted at the beginning (index 0)"""
        set_rng(default_rng(100))  # Seed chosen to insert at beginning
        ...
        # Try multiple times to potentially hit the beginning
        inserted_at_beginning = False
        for seed in range(100):
            set_rng(default_rng(seed))
            mutant_gene = mutagen.mutate(original_gene)
            
            # Check if inserted at beginning
            if isinstance(mutant_gene.child_genes[0], RotateCells) and \
               mutant_gene.child_genes[0].direction == RotateCells.Direction.FORWARD:
                inserted_at_beginning = True
                break
        
        self.assertTrue(inserted_at_beginning, "Gene should be insertable at index 0")

    def test_insertion_at_end(self):
        """Test that genes can be inserted at the end (index len(genes))"""
        set_rng(default_rng(200))
        child_genes: List[Gene] = [
            RotateCells(RotateCells.Direction.BACKWARD),
            RotateCells(RotateCells.Direction.BACKWARD)
        ]
        ...
        # Try multiple times to potentially hit the end
        inserted_at_end = False
        for seed in range(100):
            set_rng(default_rng(seed))
            mutant_gene = mutagen.mutate(original_gene)
            
            # Check if inserted at end
            if isinstance(mutant_gene.child_genes[-1], RotateCells) and \
               mutant_gene.child_genes[-1].direction == RotateCells.Direction.FORWARD:
                inserted_at_end = True
                break
        
        self.assertTrue(inserted_at_end, "Gene should be insertable at end (index len(genes))")

    def test_insertion_in_middle(self):
        """Test that genes can be inserted in the middle"""
        set_rng(default_rng(300))
        child_genes: List[Gene] = [
            RotateCells(RotateCells.Direction.BACKWARD),
            RotateCells(RotateCells.Direction.BACKWARD),
            RotateCells(RotateCells.Direction.BACKWARD)
        ...
        # Try multiple times to hit the middle
        inserted_in_middle = False
        for seed in range(100):
            set_rng(default_rng(seed))
            mutant_gene = mutagen.mutate(original_gene)
            
            # Check if inserted in middle (not first or last)
            forward_indices = [i for i, g in enumerate(mutant_gene.child_genes) 
                             if isinstance(g, RotateCells) and g.direction == RotateCells.Direction.FORWARD]
            if forward_indices and 0 < forward_indices[0] < len(mutant_gene.child_genes) - 1:
                inserted_in_middle = True
                break
        
        self.assertTrue(inserted_in_middle, "Gene should be insertable at middle indices")
