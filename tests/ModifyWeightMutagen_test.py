...
from roxene.mutagens import ModifyWeightMutagen, WeightLayer
...
class ModifyWeightMutagen_test(unittest.TestCase):
    ...
    def test_modify_weight_input_hidden(self):
        """Test that ModifyWeightMutagen modifies input_hidden weights"""
        set_rng(default_rng(SEED))
        original_gene = CreateNeuron(**random_neuron_state(10, 10, 10))
        
        mutagen = ModifyWeightMutagen(WeightLayer.input_hidden, 0.5, 0)
    ...
    def test_modify_weight_hidden_feedback(self):
        """Test that ModifyWeightMutagen modifies hidden_feedback weights"""
        set_rng(default_rng(SEED))
        original_gene = CreateNeuron(**random_neuron_state(10, 10, 10))
        
        mutagen = ModifyWeightMutagen(WeightLayer.hidden_feedback, 0.5, 0)
        mutant_gene = mutagen.mutate(original_gene)
    ...
    def test_modify_weight_feedback_hidden(self):
        """Test that ModifyWeightMutagen modifies feedback_hidden weights"""
        set_rng(default_rng(SEED))
        original_gene = CreateNeuron(**random_neuron_state(10, 10, 10))
        
        mutagen = ModifyWeightMutagen(WeightLayer.feedback_hidden, 0.5, 0)
        mutant_gene = mutagen.mutate(original_gene)
    ...
    def test_modify_weight_hidden_output(self):
        """Test that ModifyWeightMutagen modifies hidden_output weights"""
        set_rng(default_rng(SEED))
        original_gene = CreateNeuron(**random_neuron_state(10, 10, 10))
        
        mutagen = ModifyWeightMutagen(WeightLayer.hidden_output, 0.5, 0)
        mutant_gene = mutagen.mutate(original_gene)
    ...
    def test_modify_weight_susceptibility(self):
        ...
        mutagen_low = ModifyWeightMutagen(WeightLayer.input_hidden, 0.01, 0)
        ...
        mutagen_high = ModifyWeightMutagen(WeightLayer.input_hidden, 0.5, 0)
    ...
    def test_persist_reload(self):
        """Test that ModifyWeightMutagen can be persisted and reloaded"""
        mutagen = ModifyWeightMutagen(WeightLayer.input_hidden, 0.025, 0.035)
        ...
        with Session(engine) as session:
            ...
            session.commit()
        with Session(engine) as session:
            reloaded = session.get(ModifyWeightMutagen, mutagen_id)
