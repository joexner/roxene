...
class Mutagens_test(unittest.TestCase):
    ...
    def test_CreateNeuronMutagen(self):
        ...
        for layer_to_mutate in CreateNeuronLayer:
            mutagen = CreateNeuronMutagen(layer_to_mutate=layer_to_mutate)
            mutant = mutagen.mutate(gene, rng)
            self.check_fraction_changed(layer_to_mutate is CreateNeuronLayer.input_initial_value,
                                        mutant.input_initial_value, gene.input_initial_value)
            self.check_fraction_changed(layer_to_mutate is CreateNeuronLayer.input_hidden, mutant.input_hidden,
                                        gene.input_hidden)
            self.check_fraction_changed(layer_to_mutate is CreateNeuronLayer.hidden_feedback, mutant.hidden_feedback,
                                        gene.hidden_feedback)
            self.check_fraction_changed(layer_to_mutate is CreateNeuronLayer.feedback_initial_value,
                                        mutant.feedback_initial_value, gene.feedback_initial_value)
            self.check_fraction_changed(layer_to_mutate is CreateNeuronLayer.feedback_hidden, mutant.feedback_hidden,
                                        gene.feedback_hidden)
            self.check_fraction_changed(layer_to_mutate is CreateNeuronLayer.hidden_output, mutant.hidden_output,
                                        gene.hidden_output)
            self.check_fraction_changed(layer_to_mutate is CreateNeuronLayer.output_initial_value,
